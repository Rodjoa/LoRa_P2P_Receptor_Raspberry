...
enum sf_t { SF7, SF8, SF9 = 9, SF10, SF11, SF12 };
...
void setupMQTT() {
    ...
    while ((rc = MQTTClient_connect(client, &conn_opts)) != MQTTCLIENT_SUCCESS) {   //En mqttpaho MQTTCLIENT_SUCCESS  es 0 (funcion se ejecuta sin error) (MQTTClient.h)
        printf("Dentro de la funcion Setup_MQTT");
...
void sendToMQTT(char* payload) {
    ...
    if(primerbyte == '2'){
        ...
        snprintf(payloadWithRSSI, sizeof(payloadWithRSSI), "%s,%d,%.2f", 
                 localPayload, rssi_lora, SNR_LoRa);
